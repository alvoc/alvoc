...
def test_initialization_correct_columns():
    ...
    """
    with patch('alvoc.core.amplicons.analyze.pd.read_csv', return_value=DataFrame(columns=['chrom', 'chromStart', 'chromEnd'])):
        # Initialization should succeed without exceptions
        analyzer = Amplicons(Path("path/to/file.bam"), Path("path/to/inserts.csv"), "AGCT"*1000, Path("/output"))
...
def test_initialization_incorrect_columns():
    ...
    """
    with patch('alvoc.core.amplicons.analyze.pd.read_csv', return_value=DataFrame(columns=['wrong1', 'wrong2'])):
        # Expect ValueError due to missing required columns
        with pytest.raises(ValueError, match="CSV file must contain 'chrom', 'chromStart', 'chromEnd' columns."):
            Amplicons(Path("path/to/file.bam"), Path("path/to/inserts.csv"), "AGCT"*1000, Path("/output"))
