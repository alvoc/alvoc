...
from alvoc.core.precompute.precompute import precompute, download_virus_data, process_reference, extract_gene_info
...
def test_precompute_with_tax_id(tmp_path, caplog):
    ...
    with patch('alvoc.core.precompute.precompute.download_virus_data') as mocked_download:
        with patch('alvoc.core.precompute.precompute.process_reference') as mocked_process:
...
def test_process_reference(tmp_path, caplog):
    ...
    with patch('alvoc.core.precompute.precompute.SeqIO.parse', return_value=iter([organism])):
        with patch('alvoc.core.precompute.precompute.extract_gene_info', return_value={"gene": (1, 100)}):
...
@pytest.mark.parametrize("features, expected", [
    ([], {}),
    ([MagicMock(type='gene', qualifiers={"gene": ["gene1"]}, location=MagicMock(start=1, end=100))], {"gene1": [1, 100]}),
])
