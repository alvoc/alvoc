...
def precompute(options: PrecomputeOptions) -> tuple[dict[str, tuple[int, int]], str]:
    """
    ...
    """
    if not (options.tax_id or options.genbank_file):
        raise ValueError("Either 'tax_id' or 'genbank_file' must be provided.")
    
    outdir_path = Path(options.outdir)
    if not outdir_path.is_dir():
        logger.info(f"Creating directory at {outdir_path}")
        outdir_path.mkdir(parents=True, exist_ok=True)

    if options.genbank_file:
        reference_file = Path(options.genbank_file)
        return process_reference(reference_file, outdir_path)  # Assuming this function exists and handles processing.
    elif options.tax_id:
        file_path = download_virus_data(options.tax_id, outdir_path, options.email,)  # Assuming this function exists.
        if file_path:
