...
def parse_snv(snv):
    ...
    Args:
        snv (str): The SNV string in the format 'A123G'.
...
def snv_name(snv):
    ...
    Args:
        snv (tuple): A tuple containing the SNV components.
...
def parse_mutation(mut):
    ...
    Args:
        mut (str): Mutation string, which could include multiple SNVs.

    Returns:
        list: A list of parsed SNVs.
    """
    muts = aa(mut) if ':' in mut else [mut]
...
def mut_in_col(pileupcolumn, mut):
    ...
    Args:
        pileupcolumn (PileupColumn): A pileup column from a BAM file.
        mut (str): The mutation to count.
...
def mut_idx(mut):
    ...
    """
    snvs = parse_mutation(mut)  # Assumes that parse_mutation doesn't require mut_lins
